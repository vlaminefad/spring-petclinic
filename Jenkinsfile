pipeline {
    ...
    stages {
        ...
        stage('deploy') {
            steps {                                
                octopusPushBuildInformation ommentParser: 'GitHub', overwriteMode: 'FailIfExists', packageId: 'mcasperson/petclinic', packageVersion: "1.0.${BUILD_NUMBER}", serverId: "${ServerId}", spaceId: "${SpaceId}", toolId: 'Default', gitUrl: "${GIT_URL}", gitCommit: "${GIT_COMMIT}"
